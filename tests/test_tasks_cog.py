...
class TestTasksCog(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_enforce_raid_voice_absences_removes_after_15_minutes(self):
        ...
        # Create a fake leader member object that will be "in voice"
        fake_leader = MagicMock()
        fake_leader.id = leader_id
        fake_leader.bot = False
        ...
        # Leader is in voice, but the absent member (555) is not
        fake_vc = FakeVoiceChannel(members=[fake_leader])
        ...
        with patch.object(tasks_cog_module.discord, "VoiceChannel", FakeVoiceChannel):
            self.bot.db.fetchall = AsyncMock(return_value=[
                {"id": raid_id, "guild_id": guild_id, "leader_id": leader_id, "thread_id": 222, "vc_id": vc_id},
            ])
            self.bot.db.get_raid_voice_channels = AsyncMock(return_value=[])
    ...
    async def test_enforce_raid_voice_absences_auto_closes_raid_when_leader_absent_10_minutes(self):
        """Test that a raid is auto-closed when the leader is absent from voice for 10+ minutes."""
        class FakeVoiceChannel:
            def __init__(self, members=None):
                self.members = members or []
        ...
        # Create a fake member in voice (not the leader)
        ...
        # Leader is NOT in voice (only fake_member is)
        fake_vc = FakeVoiceChannel(members=[fake_member])
        mock_guild.get_channel = MagicMock(return_value=fake_vc)
        self.bot.get_guild = MagicMock(return_value=mock_guild)
        ...
        with patch.object(tasks_cog_module.discord, "VoiceChannel", FakeVoiceChannel):
            self.bot.db.fetchall = AsyncMock(return_value=[
                {"id": raid_id, "guild_id": guild_id, "leader_id": leader_id, "thread_id": thread_id, "vc_id": vc_id, "announcement_message_id": None},
            ])
            self.bot.db.get_raid_voice_channels = AsyncMock(return_value=[])
            self.bot.db.get_raid_members = AsyncMock(return_value=[{"user_id": 555}])
            self.bot.db.is_raid_member_excluded = AsyncMock(return_value=False)
            ...
            # Verify raid was deactivated
            self.bot.db.execute.assert_any_call(
                "UPDATE raids SET is_active = 0 WHERE id = ?",
                (raid_id,),
            )
            # Verify timed DKP was disabled
    ...
    async def test_enforce_raid_voice_absences_clears_leader_absence_when_leader_returns(self):
        ...
        with patch.object(tasks_cog_module.discord, "VoiceChannel", FakeVoiceChannel):
            ...
            # Verify leader absence was cleared
            self.bot.db.execute.assert_any_call(
