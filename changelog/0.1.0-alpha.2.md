...
- Moved most welcome controls into an **Open DKP Panel** button that opens a per-user **ephemeral** panel.
- The panel now hosts most common user actions (My DKP, Auction Help, Bot Status) plus privileged actions.

## Docs viewer (in-bot)

- Added an in-bot documentation viewer backed by `Documentation/*.md`.
- Docs access is **admin-only** (via the DKP Panel or `/docs`).

## Optional PostHog analytics

- Added optional PostHog event instrumentation.
- Analytics are disabled unless `POSTHOG_API_KEY` is set.

## CI / QA

- Added staging Playwright smoke tests for the changelog UI and improved reliability.
- Added Qase reporting for Playwright test runs.
